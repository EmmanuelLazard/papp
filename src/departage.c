/*
 * departage.c: calcule le departage d'un joueur avec les regles du Brightwell
 *
...
/*
 * void DepartageJoueur(BQ_resultats *TabJ,
 *                      long BQ_ronde,
 *                      double *Dep,
 *                      pions_t *TotalPions,
 *                      long *SO)
 ...
 *
 * tabJ       : tableau des resultats du joueur avec le numero FFO
 *              de l'adversaire (0 si Bip ou pas joue), le score et
 *              le total de points actuel de l'adversaire;
 * BQ_ronde   : numero de la ronde a laquelle calculer le departage (>=0);
...
void DepartageJoueur(
    BQ_resultats *TabJ, long BQ_ronde, double *Dep, pions_t *TotalPions, long *SO) {
    ...
    *SO = 0 ;
    *Dep = 0.0 ;
    *TotalPions = ZERO_PION ;
    assert ((BQ_ronde >= 0) && (BQ_ronde < ronde));
    for (i=0 ; i<=BQ_ronde ; i++) {
        if (!EGALITE_SCORES(TabJ->score[i], SCORE_INCONNU))
            AJOUTE_SCORE(*TotalPions, TabJ->score[i]) ;
        *SO += TabJ->points_adv[i] ;
        adv = numero_inscription(TabJ->adv[i]) ;
        if ((adv != -1) && present[adv]) {
        /* on a joue un adversaire reel et il est encore present dans le tournoi */
            *Dep += coef_brightwell * (float) TabJ->points_adv[i];
            if (!BUCHHOLZ_ONLY)
                *Dep += SCORE_EN_FLOTTANT(TabJ->score[i]);
        } else {
    ...
    *Dep += coef_brightwell * ((float) PartiesPasJouees *  TabJ->points_joueur) ;
    if (!BUCHHOLZ_ONLY)
        *Dep += (float) (PartiesPasJouees * total_pions/2);

    if (coef_brightwell == 0) { /* Departage aux pions */
        *Dep = SCORE_EN_FLOTTANT(*TotalPions) ;
    }
}
